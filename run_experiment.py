...
def main(argv: list[str] | None = None):
    ...
    if cfg.dry_run:
        ...
        if cfg.method == "orca_otdd":
            ...
            print("dry_run:", DatasetDistance, "CUDA?", torch.cuda.is_available())
        else:
    ...
    from orca_rank.training.gpu_utils import cuda_supports_bf16_autocast

    bf16_trimmed_for_gpu = False
    if cfg.bf16 and torch.cuda.is_available() and not cuda_supports_bf16_autocast():
        cfg = dataclasses.replace(cfg, bf16=False)
        bf16_trimmed_for_gpu = True

    meta = cfg.to_jsonable()
    if bf16_trimmed_for_gpu:
        meta["bf16_autocast_disabled_reason"] = (
